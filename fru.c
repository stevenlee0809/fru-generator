...
static void _fru_bin_append_header_and_areas(struct fru_bin *bin,struct fru_bin *chassis,struct fru_bin *board,
                          struct fru_bin *product)
{
        assert(bin != NULL && chassis != NULL && board != NULL && product != NULL);
        ...
        if(product->length == 0)
                ...
                hdr.product = (sizeof(hdr) + chassis->length + board->length) >> 3;
...
static void fru_bin_append_header_and_areas(struct fru_bin *bin,struct fru_bin *chassis,
...
static void custom_field_create_by_string(struct fru_bin **custom_field,const char **string)
{
        int i;
        for(i=0;i<OPENBMC_VPD_KEY_CUSTOM_FIELDS_MAX;i++){
                if(string[i] == NULL)
...
struct fru_area_chassis_info *fru_area_chassis_info_create_by_string(struct chassis_info *info)
{
        ...
        chassis->part_number = fru_area_field_create_by_string(info->part_number);
        chassis->serial_number = fru_area_field_create_by_string(info->serial_number);

        custom_field_create_by_string(chassis->custom_field,info->custom_field);
...
struct fru_area_product_info *fru_area_product_info_create_by_string(struct product_info *info)
{
        ...
        product->manufacturer = fru_area_field_create_by_string(info->manufacturer);
        product->product_name = fru_area_field_create_by_string(info->product_name);
        product->part_number = fru_area_field_create_by_string(info->part_number);
        product->version= fru_area_field_create_by_string(info->version);
        product->serial_number = fru_area_field_create_by_string(info->serial_number);
        product->asset_tag= fru_area_field_create_by_string(info->asset_tag);
        product->fru_file_id= fru_area_field_create_by_string(info->fru_file_id);

        custom_field_create_by_string(product->custom_field,info->custom_field);
        return product;
}
                ...
                struct board_info *board_info,struct product_info *product_info)
{
        ...
        if(chassis_info != NULL){
                ...
                fru_fru_area_chassis_info_append(chassis,fru_area_chassis_info);
        ...
        if(product_info != NULL){
                ...
                fru_fru_area_product_info_append(product,fru_area_product_info);
