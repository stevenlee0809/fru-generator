...
#define ERROR_FIELD(area,field)          \
        do{                             \
                fprintf(stderr,area" "field " field error,check the json file!\n"); \
                return -1;              \
        }while(0)
...
static int custom_field_init_by_json(const char **field,cJSON *json)
{
        ...
        array_size = array_size < OPENBMC_VPD_KEY_CUSTOM_FIELDS_MAX?
                     array_size : OPENBMC_VPD_KEY_CUSTOM_FIELDS_MAX;
        int i;
...
static int chassis_info_init_by_json(struct chassis_info *chassis,cJSON *json)
{
        ...
        chassis->part_number = cJSON_GetStringValue(
                        cJSON_GetObjectItem(json,"part_number"));
...
static int board_info_init_by_json(struct board_info *board,cJSON *json)
{
        ...
        board->mfg_time = cJSON_GetStringValue(
                        cJSON_GetObjectItem(json,"mfg_time"));
        board->manufacturer= cJSON_GetStringValue(
                        cJSON_GetObjectItem(json,"manufacturer"));
        board->product_name= cJSON_GetStringValue(
                        cJSON_GetObjectItem(json,"product_name"));
        board->serial_number= cJSON_GetStringValue(
                        cJSON_GetObjectItem(json,"serial_number"));
        board->part_number= cJSON_GetStringValue(
                        cJSON_GetObjectItem(json,"part_number"));
        board->fru_file_id= cJSON_GetStringValue(
                        cJSON_GetObjectItem(json,"fru_file_id"));
...
static int product_info_init_by_json(struct product_info *product,cJSON *json)
{
        ...
        product->manufacturer= cJSON_GetStringValue(
                        cJSON_GetObjectItem(json,"manufacturer"));
        product->product_name= cJSON_GetStringValue(
                        cJSON_GetObjectItem(json,"product_name"));
        product->part_number= cJSON_GetStringValue(
                        cJSON_GetObjectItem(json,"part_number"));
        product->version= cJSON_GetStringValue(
                        cJSON_GetObjectItem(json,"version"));
        product->serial_number= cJSON_GetStringValue(
                        cJSON_GetObjectItem(json,"serial_number"));
        product->asset_tag= cJSON_GetStringValue(
                        cJSON_GetObjectItem(json,"asset_tag"));
        product->fru_file_id= cJSON_GetStringValue(
                        cJSON_GetObjectItem(json,"fru_file_id"));
...
static int bin_generator(const char *filename,cJSON *json)
{
        ...
        if(chassis == NULL)
                ERROR_FIELD("chassis","whole");
        cJSON *board = cJSON_GetObjectItem(json,"board");
        if(board == NULL)
                ERROR_FIELD("board","whole");
        cJSON *product = cJSON_GetObjectItem(json,"product");
        if(product == NULL)
                ERROR_FIELD("product","whole");

        struct chassis_info chassis_info;
        struct board_info board_info;
        struct product_info product_info;

        chassis_info_init_by_json(&chassis_info,chassis);
        board_info_init_by_json(&board_info,board);
        product_info_init_by_json(&product_info,product);

        fru_bin_generator_by_info(filename,&chassis_info,&board_info,&product_info); 
        return 0;
}

void usage(const char *name)
{
        fprintf(stdout,"Usge: %s -j [fru.jon] -b [fru.bin]\n",name);
        exit(-1);
}
